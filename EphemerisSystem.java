...
public abstract class EphemerisSystem {
    ...
    public SatellitePosition computeSatPositionAndVelocities(long unixTime, double Pseudorange, int satID, int satType, EphGps eph, double receiverClockError) {


        if (satType != GNSSConstants.SYSTEM_GLONASS) {  // other than GLONASS

            //System.out.println("### other than GLONASS data");

            // Compute satellite clock error
            double satelliteClockError = computeSatelliteClockError(unixTime, eph, Pseudorange);

            // Compute clock corrected transmission time
            double tGPS = computeClockCorrectedTransmissionTime(unixTime, satelliteClockError, Pseudorange);
...
//		velDotArray.print();
    ...
    protected double computeSatelliteClockError(long unixTime, EphGps eph, double obsPseudorange) {
        ...
        if (eph.getSatType() == GNSSConstants.SYSTEM_GLONASS) {   // In case of GLONASS
